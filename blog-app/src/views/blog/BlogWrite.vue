    ...
    <el-container>
      <base-header :simple=true>
        <el-col :span="4" :offset="2">
          <div class="me-write-info">写博客</div>
        </el-col>
...
<!--          </el-form-item>-->
          <el-form-item label="博客分类" prop="category">
            <el-select v-model="articleForm.category" value-key="id" placeholder="请选择博客">
    ...
    methods: {
      getArticleById(id) {
        ...
        getArticleById(id).then(data => {
          ...
          if (error !== 'error') {
            that.$message({type: 'error', message: '博客加载失败', showClose: true})
          }
        })
      },
      publishShow() {
          ...
          if (valid) {
            ...
            publishArticle(article,this.$store.state.token).then((data) => {
              if(data.success){
                ...
                that.$message({message: error, type: '博客发布失败:'+data.msg, showClose: true});
              }
      ...
      cancel() {
        this.$confirm('已编辑内容将不会保存, 是否继续?', '提示', {
